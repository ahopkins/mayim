...
_global_transaction: ContextVar[Optional['GlobalTransactionContext']] = ContextVar(
    "global_transaction", default=None
)
...
class GlobalTransactionContext:
    ...
    async def add_executor(self, executor):
        ...
        if not isinstance(executor, SQLExecutor):
            raise MayimError(f"Only SQL executors can participate in transactions, got {type(executor)}")
...
class TransactionCoordinator:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pool_count': len(self._context.connections) if self._context else 0,
        }
